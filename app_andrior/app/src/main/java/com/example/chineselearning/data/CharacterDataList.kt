...
object CharacterDataList {
    val characters = listOf(
        ...
        CharacterData(
            ...
            meaning = "speak, say",
            strokes = 9,
            examples = "请说中文。(Please speak Chinese.)",
            lastReviewTime = 0L,
            nextReviewTime = 0L,
            reviewCount = 0,
            masteryLevel = 0,
    ...
    // 根据级别获取汉字
    fun getCharactersByLevel(level: Int): List<CharacterData> {
        return characters.filter { it.id <= level * 20 && it.id > (level - 1) * 20 }
    }

    // 获取特定ID的汉字
    fun getCharacterById(id: Int): CharacterData? {
